...
void kernel_main() {
    ...
    DPRINT << "Up sdfg_3_tenstorrent_kernel_599.combined" << ENDL();

    cb_reserve_back(0, 1);
    cb_reserve_back(1, 1);
    cb_reserve_back(2, 1);
    uint32_t tt_last_unit = tt_first_unit + tt_work_units;
    _13_tile0 = tt_first_unit*1024;
    ...
    for(uint32_t tt_tile_idx = tt_first_unit; tt_tile_idx < tt_last_unit; ++tt_tile_idx) {
...

        {
            ...
            // Map
            for(tt_idx_within_block = 0, _13 = _13_tile0; (_13 < _1 && _13 < 1024 + _13_tile0); tt_idx_within_block += 1, _13 = 1 + _13)
            {
                    {
                        float _in2 = (reinterpret_cast<float *>(__daisy_tt__5))[tt_idx_within_block];
                        float _in3 = (reinterpret_cast<float *>(__daisy_tt__3))[tt_idx_within_block];
                        float _in1 = _4;
                        float _out;
