...
void kernel_main() {
    ...
    float _4 = get_arg_val<float >(4);
    uint32_t tt_tensor_addr_0 = get_arg_val<uint32_t>(5); // base addr of __daisy_tt__3
    uint32_t tt_tensor_addr_1 = get_arg_val<uint32_t>(6); // base addr of __daisy_tt__5
    uint32_t tt_tensor_addr_2 = get_arg_val<uint32_t>(7); // base addr of __daisy_tt__6
    ...
    DPRINT << "Up sdfg_3_tenstorrent_kernel_657.combined" << ENDL();

    cb_reserve_back(0, 1);
    cb_reserve_back(1, 1);
    cb_reserve_back(2, 1);
    uint32_t tt_last_unit = tt_first_unit + tt_work_units;
    _43_tile0 = tt_first_unit*1024;
    ...
    for(uint32_t tt_tile_idx = tt_first_unit; tt_tile_idx < tt_last_unit; ++tt_tile_idx) {
...

        {
            ...
            // Map
            for(tt_idx_within_block = 0, _43 = _43_tile0; (_43 < _1 && _43 < 1024 + _43_tile0); tt_idx_within_block += 1, _43 = 1 + _43)
            {
                    {
                        float _in2 = (reinterpret_cast<float *>(__daisy_tt__5))[tt_idx_within_block];
                        float _in1 = _4;
                        float __out;
