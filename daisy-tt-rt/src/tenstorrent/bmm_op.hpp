...
static inline std::vector<uint32_t> get_prime_factors(uint32_t n) {
    uint32_t i = 2;
...
static inline std::vector<uint32_t> get_possible_products(std::vector<uint32_t> factors) {
    if (factors.size() == 0) {
        return {1};
    }
...
static inline CoreCoord get_core_range(
